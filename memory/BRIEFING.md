...
- **Runtime routing validation (P6) — `done`**: real Codex 0.133.0 picks up the matrix install via project-local `.agents/skills/` and routes prompts to the right leaf across 6 pack repos.
- **Phase B2 closed** (queue empty).

## Decision Log (recent)

- P6 (2026-05-28): End-to-end runtime routing of the matrix install verified against real Codex CLI 0.133.0. Set up `/tmp/ml-matrix-test-project/`, ran chain installer to populate `.agents/skills/` (6 profile adapters + 74 leaves), then ran 8 `codex exec --ephemeral --sandbox read-only --skip-git-repo-check` probes with `model_reasoning_effort=low`. Findings: (1) Codex 0.133 discovers project-local `.agents/skills/` from cwd — no global install needed for matrix testing. (2) 6/6 real routing prompts picked the correct leaf, including 2 confusable disambiguation cases (`experiment-design-planner` vs `run-experiment`/`result-diagnosis`; `project-pivot-planner` vs `result-diagnosis`/`research-idea-validator`). (3) Routing crosses pack boundaries cleanly: prompts were routed to leaves living in 3 different sibling pack repos (`quick-experiment-skills`, `paper-reading-skills` or `research-distillation-skills`, `ml-research-skills`). (4) Cross-pack dedup works: enumeration probe for `reference-*` returned exactly the 4 dirs on disk. (5) One counting probe returned a wrong integer (`research-project-memory` count = 2 instead of 1) — verified to be LLM low-reasoning hallucination, not a real dedup failure (disk + enumeration confirm 1 copy). No test artifact committed; the probes used the user's Codex credentials so the harness is operator-driven, not skill-os tests.
